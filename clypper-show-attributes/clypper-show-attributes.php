<?php
/*
Plugin Name: Clypper's Show Attributes
...
class Clypper_Show_Attributes {
    ...
    private function get_attributes($product, $single_product): array {
        ...
        // Initialize terms_by_taxonomy as empty array if false
        if (false === $terms_by_taxonomy) {
            $terms_by_taxonomy = []; // Initialize as empty array
        ...
        // Get attributes once instead of twice
        $product_attributes = $product->get_attributes();
        $attributes = [];
    ...
    public function clypper_additional_information() {
        ...
        <div class="attribute-wrapper">
            <?php
            foreach ($product_attributes as $attribute) {
                $this->display_attribute_item($attribute, $terms_by_taxonomy);
            }
            ?>
        </div>
        <?php
    ...
    private function display_attribute_item($attribute, array $terms_by_taxonomy) {
        $label = esc_html(wc_attribute_label($attribute->get_name())) . ':';
        ?>
        <div class="attribute-item">
            <p class="attribute-name"><strong><?php echo $label; ?></strong></p>
            <div class="attribute-value-wrapper">
                <?php $this->display_attribute_values($attribute, $terms_by_taxonomy); ?>
            </div>
        </div>
        <?php
    }

    private function display_attribute_values($product_attribute, array $terms_by_taxonomy) {
        $output = [];
        ...
        if ($product_attribute->is_taxonomy()) {
            ...
            foreach ($attribute_values as $term) {
                ...
                $output[] = "<p class='attribute-value-single'>{$name}</p>";
            }
        } else {
            $attribute_values = $product_attribute->get_options();

            foreach ($attribute_values as $attribute_value) {
                $attribute = esc_html($attribute_value);

                $output[] = "<p class='attribute-value-single'>{$attribute}</p>";
            }
        }
