<?php
	...
	if ( ! is_admin() || defined( 'DOING_AJAX' ) ) {
		...
		function clypper_cart_override( $product_price, $cart_item, $cart_item_key ) {
			...
			$tax_value         = wc_price( wc_get_price_including_tax( $_product ) * 1 );
			$product_quantity  = $cart_item['quantity'];
			$value_stripped    = wc_get_price_including_tax( $_product ) * 1 * $product_quantity;
			$value_stripped_ex = wc_get_price_excluding_tax( $_product ) * 1 * $product_quantity;
			$value_stripped    = wc_format_decimal( $value_stripped, $dp );
			$value_stripped_ex = wc_format_decimal( $value_stripped_ex, $dp );

			$wootax_text = get_option( 'clypper-tax_text', 'VAT' );
			$clypper_incl     = __( 'Incl.', 'clypper-tax' );
			$clypper_excl     = __( 'Excl.', 'clypper-tax' );

			if ($wootax_text == '') {
				$wootax_text = __('VAT', 'clypper-tax');
			}

			$tax_status = $_product->is_taxable();
			$product_tax_class = $_product->get_tax_class();

			$wootax_incl_message = '<span class="wootax-suffix">' . $clypper_incl . ' ' . $wootax_text . '</span>';
			$wootax_excl_message = '<span class="wootax-suffix">' . $clypper_excl . ' ' . $wootax_text . '</span>';
		 ...
		 * Cart Prices
		 */
		function clypper_cart_price_html( $product_price, $cart_item, $cart_item_key ) {

			global $wootax_text, $clypper_incl, $clypper_excl;
			...
			if ( is_cart() ) {
				...
				return '<span style="display:none;" class="amount product-tax-on product-tax">' . wc_price( $value_stripped ) . $clypper_incl . $wootax_text . '</span><span class="amount product-tax-off product-tax">' . wc_price( $value_stripped_ex ) . '</span>';
			}
		}
		add_filter( 'woocommerce_cart_item_price', 'clypper_cart_price_html', 100, 3 );

		/**
		 * Variation Prices
		 */
		function clypper_variation_price_html( $price, $variation ) {

			global $wootax_text, $wootax_incl_message, $wootax_excl_message, $clypper_incl, $clypper_excl;

			$dp                = wc_get_price_decimals();
			$value_stripped    = wc_get_price_including_tax( $variation ) * 1;
			$value_stripped_ex = wc_get_price_excluding_tax( $variation ) * 1;

			return '<span style="display:none;" class="amount product-tax-on product-tax">' . wc_price( $value_stripped ) . ' ' . $clypper_incl . $wootax_text . '</span><span class="amount product-tax-off product-tax">' . wc_price( $value_stripped_ex ) . ' ' . $wootax_excl_message . '</span>';
		}
		 ...
		 */
		function clypper_variation_sale_price_html( $price, $variation ) {
		...
		function clypper_cart_subtotal_html( $cart_subtotal, $compound, $cart ) {
			...
			if ( is_cart() || is_checkout() || defined( 'WOOCOMMERCE_CHECKOUT' ) ) {
				...
				return '<span style="display:none;" class="amount product-tax-on product-tax" title="With tax added">' . wc_price( $value_stripped ) . ' ' . $clypper_incl . $wootax_text . '</span><span class="amount product-tax-off product-tax" title="With tax removed">' . wc_price( $value_stripped_ex ) . '</span>';
			}
		}
